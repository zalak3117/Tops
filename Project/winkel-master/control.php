...
class control extends model // step 2 extends model
{
	...
	function __construct()
	{
		...
		switch($path)
		{
			...
			case '/product-single':
			include_once('product-single.php');
			break;
			...
			if(isset($_REQUEST['btncust_id']))
			{
				...
				if(isset($_REQUEST['submit']))
				{
					...
					{
						$data=array("name"=>$name,"address"=>$address,"mobile"=>$mobile,"gender"=>$gender,"language"=>$language,"birthdate"=>$birthdate,"image"=>$image);
					...
					{
						$data=array("name"=>$name,"address"=>$address,"mobile"=>$mobile, "gender"=>$gender,"birthdate"=>$birthdate,"language"=>$language);
						$res=$this->update_where('customer',$data,$where);
						if($res)
						{
							echo "
			...
			if(isset($_REQUEST['submit']))
			{
				...
				$data=array("name"=>$name,"address"=>$address,"mobile"=>$mobile,"email"=>$email,"password"=>$enc_password,"gender"=>$gender,"language"=>$language,"birthdate"=>$birthdate, "image"=>$image);
